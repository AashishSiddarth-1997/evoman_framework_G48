...
def fitness(individual):
    ENV.play(pcont=individual)
    return ENV.fitness_single()

def evaluate(population):
    return np.array(list(map(lambda y: fitness(y), population)))

if __name__ == '__main__':
    if not os.path.exists(E_NAME + '/evoman_solstate'):
        print( '\nNEW EVOLUTION\n')
        ...
        ini_g = 0
        print("Best fitness: ", fit_population[best])
        print("Mean fitness: ", mean)
        print("Std fitness: ", std)
